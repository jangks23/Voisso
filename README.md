...
- **`./scripts/test.sh`** — 흩어진 테스트를 한 번에 돌린다. **키가 없어서 못 도는 검사는
  ...
  `--ci` 는 네트워크·실데이터·키 의존을 전부 빼고, `--network` 는 실제 크롤링까지,
  `--live` 는 외부 API 실호출(TTS·STT·LLM)까지 포함한다. **`--live` 는 기본이 아니다** —
  음성 합성은 유료라서 명시적으로 켤 때만 돈다.
...
> **사투리 정규화는 사람이 아니라 기계를 위한 것이다.** 공무원은 "물이 안 빠져가"를 그냥
> 알아듣는다. 못 알아듣는 쪽은 **검색 계층**이다 — 받아쓴 문장이 그대로 검색어가 되므로,
> 어긋나면 사무분장 원문과 매칭되지 않아 **엉뚱한 과로 배정된다.** 그래서 표준어로 되돌린
> 다음 찾는다. 사투리 문장과 표준어 문장이 같은 부서로 가는지는 회귀 테스트로 확인한다.
...
- 통화 화면(어르신) → http://localhost:8000/call/
- 담당자 대시보드 → http://localhost:8000/dashboard/
- **발표용 분할 화면 → http://localhost:8000/demo/** — 위 둘을 한 화면에 나란히 띄운다
...
정직하게 적는다. **아래 표에서 ✅ 표시된 것만 지금 동작한다.**
...
| 사투리 정규화 / 변환 (어휘 382개 · 어미 규칙 71개) | ✅ 동작 | 위 4단계 · `python3 -c "from voisso.dialect import lexicon_size; print(lexicon_size())"` |
...
| 담당자 핸드오프 — 서버 API | ✅ 동작 | 4개 엔드포인트가 붙었고 양쪽 말투 변환까지 확인했다 ([아래 절](#담당자-핸드오프--사람이-이어받는다)) |
| 담당자 핸드오프 — 화면 (통화·대시보드) | ⚠️ 연결됨 · 실동작 미확인 | 양쪽 화면에 코드가 붙었다. **브라우저에서 눈으로 확인한 것은 P7/P8 몫**이라 여기서는 단정하지 않는다 |
...
| 진행 안내 콜백 — 수신 화면 | ⚠️ 연결됨 · 실동작 미확인 | 수신 화면과 받기 버튼(`btnAnswer` → `/api/callback/{id}/answer`)이 붙었다. 브라우저 확인은 P7/P8 |
| 어르신 화면 자막 표시 | 🚧 **작업 중** | `index.html` 에 자리(`#captions`)는 있지만 `app.js` 가 아직 채우지 않는다 |
| 발표용 분할 화면 (`/demo`) | ✅ 동작 | `http://localhost:8000/demo/` — 좌 35% 통화 · 우 65% 대시보드 |
...
| `--audio` | TTS·STT 를 켜고 **합성한 어르신 음성을 실제로 보내** 돈다. **외부 API 비용이 발생하므로 기본은 꺼져 있다** (합성 결과는 캐시해 재사용한다). |
...
**어르신 모드는 전화기다. 채팅창이 아니다.** 화면 대부분을 통화 상태·상대 표시·큰 말하기
버튼이 차지하고, 방금 오간 말은 **자막 한두 줄**로 지나간다. 대화 기록을 목록으로 쌓지
않는다 — **실제 전화에서 어르신은 대화 기록을 읽지 않는다. 듣고 말할 뿐이다.**
전화 화면에 채팅 로그가 쌓이면 "이걸 읽어야 하나" 하는 부담이 생기고, 그 순간 전화라는
메타포가 무너진다.

| | 어르신 모드 (기본) | 시연 모드 (`?debug=1`) |
|---|---|---|
| 화면의 뼈대 | **전화 UI** — 통화 상태 · 상대 표시 · 큰 말하기 버튼 | 전화 UI + 진단 패널 |
| 오간 말 | **자막 한두 줄** (지나간다) | 전사 3단 대비 전체가 남는다 |
| 본문 글자 | 최소 **20px** | 15px |
| 통화 버튼 | 화면 폭 92%, 높이 **200px** | 일반 크기 |
| 말하기 버튼 | 한 줄 전체, 높이 **96px**, 글자 병기 | 아이콘 버튼 |
| 슬롯 패널 · 타이머 · 테마/글자크기 토글 | **숨김** | 표시 |
| 하단 진단 막대 | 없음 | `서버 · 입력경로 · 슬롯 3/4 · 응답 1.4초 · 담당자 연결됨` |
| 상태 표시 | 색이나 아이콘이 아니라 **글자로** ("듣고 있습니더") | 동일 + 진단 정보 |

### 전사 3단 대비는 시연 모드에 있다

우리 데모의 핵심 증거는 **받아쓴 원문 → 어르신이 실제로 한 말 → 표준어로 고친 결과**를
나란히 놓는 것이다. 어르신 화면에서는 이걸 걷어냈지만 **없앤 게 아니라 옮겼다.**
서버가 매 턴 `caller_turn` 으로 세 값을 다 내려준다.

```jsonc
"caller_turn": {
  "stt_raw":  "비가 오면 집앞 도로에 물이 안 빠져가 큰일이라예",   // 받아쓴 원문
  "dialect":  "비가 오면 집앞 도로에 물이 안 빠져가 큰일이라예",   // 어르신이 한 말
  "standard": "비가 오면 집앞 도로에 물이 안 빠져서 큰일이에요",   // 정규화 결과
  "source": "text", "stt_provider": null
}
```

> 🚧 **어르신 화면의 자막 표시는 P7이 작업 중이다.** 전화 UI 전환과 함께 다듬고 있다.

```bash
http://localhost:8000/call/            # 어르신 모드 — 어르신에게 보여줄 화면
http://localhost:8000/call/?debug=1    # 시연 모드 — 심사·발표에서 내부 동작을 보여줄 화면
http://localhost:8000/demo/            # 발표용 분할 화면 — 어르신 화면 + 담당자 화면
```
...
혼자 시연할 때 창 두 개를 손으로 배치하지 않아도 되게 만든 화면이다.

- **좌 35% 어르신 화면 / 우 65% 담당자 화면.** 경계선을 끌어 비율을 바꿀 수 있다.
- 상단에 **6단계 표시** — ① 접수 ② 배정 ③ 민원카드 ④ 대시보드 ⑤ 담당자 연결 ⑥ 진행 안내
- 상단 **「담당자 연결」** 버튼 — 대시보드를 따로 조작하지 않아도 ⑤·⑥ 단계가 흐른다
...
> **서버 API는 동작한다** — 아래 `curl` 예시는 실제로 실행해 얻은 출력이다.
> 통화 화면과 대시보드에도 코드가 붙었지만, **브라우저에서 눈으로 확인한 것은 P7/P8 몫**이라
> 이 문서에서는 단정하지 않는다. 계약은 [`docs/CONTRACT.md`](docs/CONTRACT.md) 5-B절.
...
**담당자가 사투리를 못 알아들어서가 아니다.** 한국 사람은 "낼 오신다 캅니꺼"를 다 알아듣는다.
**어르신이 하던 대로 말하게 두려는 것이다** — 기계 앞에서 말을 고르던 사람이 이제 사람과
편하게 이야기한다. 어르신이 한 말은 원문 그대로 전달되고, 표준어 변환은 **검색과 기록**에 쓴다.
...
그래서 응급으로 판정되면 접수보다 119가 먼저다. **다만 통보하지 않고 물어본다.**
아래는 실제 출력이다.

```
어르신: 지금 집에 물이 차오르고 있어예 어무이가 안에 계시는데예
Voisso: 제가 119에 연결해 드릴까예?
어르신: 네
Voisso: 예, 119로 연결하겠습니더.
```

처음에는 "위험하시면 119에 전화해 주이소"라고 **통보**했다. 그러면 어르신이 직접 번호를
눌러야 한다. 지금은 **우리가 걸어 드리겠다고 묻고 말로 확인받는다.**

**이유는 이 프로젝트의 전제 그 자체다 — 어르신은 버튼보다 말이 편하다.** 그 전제 위에
서비스를 만들어 놓고 정작 가장 급한 순간에만 "버튼을 누르세요"라고 하면 앞의 모든 주장이
무너진다. **안전 기능도 같은 원칙을 따른다.**

확인을 받으면 `urgency.safety_referral.confirmed` 가 `true` 가 된다. 거절하면 강요하지
않고("예, 알겠습니더. 위험해지시면 언제든 말씀해 주이소"), 두 번 물어도 불분명하면 더
묻지 않는다 — **겁을 주면 안 되기 때문이다.**

> **브라우저 제약은 정직하게 밝힌다.** 브라우저는 사용자가 직접 누르지 않으면 `tel:` 을
> 실행하지 못한다. 말로 "네"라고 한 것은 브라우저 입장에서 조작이 아니다. 그래서 확인
> 뒤에도 **화면의 큰 단추를 한 번 누르는 절차가 남는다.** 우리 설계의 한계가 아니라
> 브라우저의 제약이고, **실제 전화망 연동(H3)에서는 사라진다.**

**접수가 신고를 대체한다고 오해하게 만들지 않는다.** 확인이 먼저, 접수는 그 다음이다.
...
**② 방언 사전(`F`)이 세 곳에 붙는다.** 들어오는 말은 **표준어로 되돌려 검색**하고
(`C` → `E`), 나가는 말은 **어르신에게 익숙한 말투로** 바꾼다 — AI 응답(`F`)뿐 아니라
담당자의 글(`K`)과 진행 안내 브리핑(`L`)에서도 같은 사전을 쓴다.
**앞은 정확도의 문제이고, 뒤는 편안함의 문제다.**
...
4. **여럿이 동시에 개발할 때는 자기 서버를 격리하라.** 포트만 나누면 부족하다 —
   데이터 디렉터리를 공유하면 서로의 민원카드가 섞이고, TTS 를 켜 두면 종량제
   크레딧을 함께 쓴다. 자세한 규칙은 [`docs/CONTRACT.md`](docs/CONTRACT.md) 5-D·5-E 절.
   ```bash
   mkdir -p /tmp/voisso-8023 && ln -sf "$PWD/data/gb_departments.json" /tmp/voisso-8023/
   VOISSO_TTS_PROVIDER=none VOISSO_DATA_DIR=/tmp/voisso-8023 python3 -m server --port 8023
   ```
   음성이 필요한 검증은 **명시적 플래그로만** 연다 — `rehearsal.py --audio`,
   `server.e2e_check --audio`, `scripts.tests.live_checks --allow-tts`.
5. **의존성을 최소로 유지하라.** 표준 라이브러리로 되는 일은 표준 라이브러리로 한다. 무거운 모델을 내려받는 코드는 받지 않는다.
6. **README를 같이 고쳐라.** 새 명령·환경변수·의존성이 생기면 같은 PR에서 README를 갱신한다. *"README만 보고 처음 보는 사람이 실행할 수 있어야 한다"* 가 이 프로젝트의 기준이다.
