...
| DEMO_SCRIPT 대비책 C·D·E·F | ❌ 4건 오류 | ✅ **P1이 전부 수정. 문서의 폴백 명령을 실제로 돌려 검증했다** |
...
## DEMO_SCRIPT 「실패 대비책」 재검증 — **이전 지적 4건 모두 수정 확인**
...
`DEMO_SCRIPT.md` 네트워크 끊김 행의 "STT 60초 + LLM 45초 + TTS 30초" 옆에
...
| 서버가 죽음 | `open web/call/index.html` (자동 목 모드, 텍스트 입력) | DEMO_SCRIPT 반영됨 |
