...
보여주는 값이다. 담당자가 납득하지 못하면 그 배정은 무시된다.
...
**양쪽 말투를 각자에게 맞춰 옮기는 것이 이 기능의 동작이다.**
...
**주장을 정확히 하라.** 한국인 담당자는 "낼 오신다 캅니꺼" 를 그냥 알아듣는다.
통역이 필요해서 만든 기능이 아니다. 이 변환이 하는 일은 두 가지이고, 그 이상을 주장하지 마라.

- **담당자 → 어르신**: 어르신에게 **익숙한 말투로** 전달한다. **편안함의 문제**다.
  기계 앞에서 말을 고르던 사람이 하던 대로 말할 수 있게 하는 것.
- **어르신 → 담당자**: 원문을 그대로 전달하되 표준어도 함께 보관한다.
  **검색·기록의 문제**다(정규화된 문장이라야 사무분장 원문과 매칭된다).

"담당자가 못 알아들어서 통역한다" 고 쓰지 마라. 심사에서 무너지는 주장이다.
...
## 5-D. 음성 생성 금지 (개발 중) — 비용 규칙

**타입캐스트 TTS 는 종량제다. 개발·테스트 중에 음성을 생성하지 마라.**
사용자가 크레딧을 충전했고, 그것은 **발표와 촬영을 위한 것**이다.
에이전트가 테스트로 소진하면 정작 시연 때 음성이 안 나온다. 실제로 한 번 그렇게 됐다.

**모든 에이전트는 자기 테스트 서버를 띄울 때 반드시 이렇게 한다.**

```bash
VOISSO_TTS_PROVIDER=none python3 -m server --port <자기 포트>
```

- 스크립트·리허설·셀프테스트는 **기본값이 음성 없음**이어야 한다.
  음성을 켜는 경로는 명시적 플래그(예: `--audio`)로만 열린다.
- `.env` 의 `VOISSO_TTS_PROVIDER=typecast` 는 **사용자 미리보기(8111)와 촬영 전용**이다.
  거기에 맞춰 자기 셸에서 환경변수를 덮어써라.
- TTS 코드를 고쳤으면 **요청 바디가 올바른지까지만 확인**하고 실제 호출은 하지 마라.
  실제 음성 확인이 꼭 필요하면 나에게 먼저 알려라.
- STT(OpenAI)와 대화(OpenAI)도 같은 원칙이다. 필요한 만큼만, 반복 검증은 캐시하라.

## 5-E. 테스트 데이터 격리 (개발 중) — 사용자 발표 데이터를 오염시키지 마라

**포트를 나눠도 데이터 디렉터리를 공유하면 소용이 없다.** `VOISSO_DATA_DIR` 기본값이
`./data` 라서, 각자 포트로 띄운 테스트 서버가 **같은 `data/complaints/` 에 카드를 쌓는다.**
실제로 발표용으로 3건만 남겨 정리한 것이 에이전트 테스트로 7건이 됐다.
사용자가 발표 화면을 열 때마다 테스트 잔해가 섞여 보인다.

**모든 에이전트는 자기 테스트 서버를 이렇게 띄운다.**

```bash
VOISSO_TTS_PROVIDER=none \
VOISSO_DATA_DIR=/tmp/voisso-<자기포트> \
python3 -m server --port <자기포트>
```

부서 데이터가 필요하면 그 디렉터리에 심볼릭 링크를 건다. **복사하지 마라** — 갱신되면 어긋난다.

```bash
mkdir -p /tmp/voisso-<자기포트>
ln -sf "$PWD/data/gb_departments.json" /tmp/voisso-<자기포트>/
```

**절대 규칙**

- **`data/complaints/`, `data/handoffs/`, `data/callbacks/` 를 직접 건드리지 마라.**
  거기는 **사용자 발표용**이다. 읽는 것도 필요할 때만, 쓰거나 지우는 것은 금지다.
  잔해를 발견하면 **지우지 말고 보고하라** — 무엇이 발표용이고 무엇이 잔해인지는
  사용자가 판단한다.
- 리허설·셀프테스트도 같은 원칙이다. **기본 데이터 디렉터리에 쓰지 않는다.**
  임시 디렉터리를 쓰고 끝나면 정리한다.
- HTTP 로 붙는 검증 도구(`server.e2e_check`, `server.dialogue_check`, `server.voice_check`)는
  **자기가 데이터 위치를 정하지 못한다.** 격리는 **서버를 띄우는 쪽**의 책임이다.
  이 도구들을 돌리기 전에 위 명령으로 띄운 자기 서버를 대상으로 잡아라(`--base`).

**포트 배정** — P3=8020, P4=8021, P5=8022, P6=8023, P7=8024, P8=8025.
**8000 사용 금지. 8111 은 사용자 미리보기 전용 — 쓰지도, 죽이지도 마라.**

관련: [5-D 음성 생성 금지](#5-d-음성-생성-금지-개발-중--비용-규칙)
...
(README 만으로 실행 가능해야 한다는 것이 이 프로젝트의 원칙이다).
...
- 완벽보다 **동작하는 것**이 우선이다. 단, mock 데이터로 끝내지 마라.
