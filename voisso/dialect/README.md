...
| `for_tts(text)` | 숫자를 낱자 한글로 — **TTS 입력 직전에만** |
...
```python
display = "위험하시믄 119 눌러 주이소."      # 자막에 그대로
speech  = for_tts(display)                 # "위험하시믄 일일구 눌러 주이소." → TTS 입력
```
...
| 054-880-2113 | 공오사 팔팔공 이일일삼 |

전화 문맥이라 **0은 "공", 6은 "육"** 이다. 묶음 사이는 공백으로 끊어 TTS 가 숨을 쉬게 한다.

**날짜·개수·시각은 건드리지 않는다.** `3일 이내`, `오전 9시`, `2주`, `1억 2천만 원` 은
한자어로 읽는 것이 맞다. 낱자로 바꾸면 오히려 못 알아듣는다.

> ⚠️ 이 표기는 **실제 음성 생성 없이** 정했다(계약서 5-D 비용 규칙). 근거는 한국어 수사
> 읽기 규칙이며, **실제 확인은 촬영 전에 해야 한다.**
