...
(function () {
  ...
  const state = {
    ...
    showStdAll: false, recording: false,
  };
  ...
  function addBubble(role, dialect, standard, opts) {
    ...
    const who = role === 'caller' ? '나 (발신자)' : 'Voisso 상담원';
    b.innerHTML =
      '<div class="bubble-who">' + esc(who) + '</div>' +
      '<div class="bubble-dialect"></div>' +
      '<div class="bubble-std"><span class="lbl">표준어 변환</span><span class="std-body"></span></div>' +
      '<button class="bubble-toggle" type="button">표준어 보기</button>';

    const toggle = b.querySelector('.bubble-toggle');
    toggle.addEventListener('click', () => {
      b.classList.toggle('show-std');
      toggle.textContent = b.classList.contains('show-std') ? '사투리 원문만 보기' : '표준어 보기';
    });
    ...
    const api = {
      ...
      set(dia, std) {
        ...
        b.querySelector('.std-body').innerHTML = markChanged(d, s);
        b.querySelector('.bubble-std .lbl').textContent =
          (d.trim() === s.trim()) ? '표준어 변환 (원문과 동일)' : '표준어 변환';
        scrollDown();
      },
    };
  ...
  function setBusy(on) {
    ...
    el.btnMic.disabled = on || !micSupported();
  ...
  async function startCall() {
    ...
    try {
      ...
      state.timer = setInterval(() => {
        el.callTimer.textContent = mmss(Math.floor((Date.now() - state.startedAt) / 1000));
      }, 500);
    ...
    if (CFG.GREETING_ON_START !== false) {
  ...
  async function sendTurn(payload, opts) {
    ...
    if (!o.silentCaller) {
      callerBubble = payload.audio_b64
        ? addBubble('caller', '(음성 전송 중…)', '(음성 전송 중…)', { pending: true })
        : addBubble('caller', payload.text, payload.text);
    }
    ...
    try {
      ...
      const callerStd = r.caller_standard || r.text_standard || r.normalized_text || null;
      if (callerBubble) {
        const dia = callerRaw || payload.text || '(음성 발화)';
  ...
  async function endCall() {
    ...
    try {
      ...
      el.delivery.querySelector('.delivery-text').textContent = '접수 실패: ' + (e.message || e);
      toast('민원 접수 실패: ' + (e.message || e), 6000);
    } finally {
  ...
  function playAudio(b64, mime) {
    ...
    try {
      ...
      if (p && p.catch) p.catch(() => {});                   // 자동재생 차단은 무시(텍스트로 이미 보인다)
  ...
  el.btnMic.addEventListener('click', () => (state.recording ? stopRecording(false) : startRecording()));
  ...
  if (!micSupported()) {
    ...
    el.btnMic.title = '이 브라우저/환경에서는 마이크를 쓸 수 없습니다. 텍스트로 진행하세요.';
  }

  (async function initModeChip() {
    if (API.isMock()) {
      el.modeChip.innerHTML = '<b>데모 모드</b> · 목 API로 동작 중 (서버 없이 전 과정 시연 가능)';
      return;
    }
