...
| 조건 | 의미 |
|---|---|
| 1순위 점수 < 0.46 | 어느 부서도 충분히 안 맞는다 |
| 1·2순위 격차 < 0.10 | 두 부서가 비등하다 — 사람이 골라야 한다 |
| `grounded: false` | 민원인이 쓴 단어가 사무분장 원문에 아예 없다 (유사어로만 찾은 결과) |

점수 임계값 0.46 은 실데이터에서 측정해 잡았다. 정상 민원 질의 8건은 0.497~0.825,
도청 소관이 아닌 질의 4건("심해 잠수정 도색", "화성 이주 신청" 등)은 0.000~0.427 로
...
| `concepts.py` | **민원 개념 사전 55개.** 구어 증상 표현 → 행정 용어, 관할(도/시군) 표시 |
...
회귀 테스트는 `python3 -m mcp_server.regression` 로 재현한다 (**59케이스 100% 통과**).
`./scripts/test.sh` 통합 러너에도 물려 있어, 라우팅이 퇴행하면 러너가 실패한다.

| 질의 | 종류 | 1순위 | 점수 | 판정 |
|---|---|---|---|---|
| 집 앞 하수구가 막혀서 물이 안 빠진다 | 표준어 | 기후환경국 맑은물정책과 | 0.664 | 후보 제시 |
| 집 앞에 물이 안 빠지고 자꾸 고여서 큰일이에요 | 구어체(행정용어 없음) | 기후환경국 맑은물정책과 | 0.653 | 후보 제시 |
| 농로가 무너졌다 | 표준어 | 농축산유통국 스마트농업혁신과 | 0.696 | 단독 배정 |
| 버스 노선을 늘려달라 | 표준어 | 경제통상국 교통정책과 | 0.744 | 단독 배정 |
| 재난지원금 신청 방법 | 표준어 | 안전행정실 자연재난과 | 0.779 | 단독 배정 |
| 우리 동네 하수구가 막혔는데 어데 전화하믄 되노? | 사투리 | 기후환경국 맑은물정책과 | 0.664 | 후보 제시 |
...
| 우리 동네 가로등이 며칠째 안 들어와요 | 시군 소관 | — | — | **시군 민원실 안내** |
| 빙하 탐사선 견인은 누가 담당하노? | 범위 밖 | — | — | **후보 없음 → 대표번호 안내** |

사투리·구어체 질의는 표준어로 고치지 않고 **원문 그대로** 넣은 결과다. 시연 대본과 근거 원문 전문은 [DEMO.md](./DEMO.md) 에 있다.
